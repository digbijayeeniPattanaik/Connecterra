...
- The application is created using .net core and EntityFramework core and databse in Microsoft SQL.
...
- There was a initial migration to the table using Microsoft.EntityFrameworkCore.Design. This was added to API project for migration.
...
- There are total of 4 APIs created with multiple operations.
...
- There is swagger implemetation for the API to know the Request and Response with status code in details. Available in Json and can also access after running the application.
...
These changes are happening on a regular basis, i.e. multiple times a day from various sources. Sometimes errors may be committed as well, e.g., cow's state can change from Pregnant to Dry and back to Pregnant within the same day. Typically cow state changes and sensor state changes occur after multiple days, so any quick changes happening in succession are generally erroneous and need to be flagged.
...
- I have used EntityFrameworkCore ChangeTracker to achieve the Audit trail of the data update and create. As part of this implementation I have created an Audit table and storing the audit details in that table whenever there is a change in state of Cow and Sensor.
